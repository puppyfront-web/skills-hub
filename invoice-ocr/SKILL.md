...
- **表**：想好要录进**哪张表**。你可以直接发 Excel，也可以说清楚文件名或路径；不要默认一定叫“面料成本核算”。
- **心理预期**：**款号、件数、成本细拆** 有的表要**你自己补**或后面再填；识别主要帮你省**厂家、品名/货号、数价金额** 这些重复劳动。

### 第一次使用建议顺序（分步指引）

1. **直接发**「我要录面料单」+ **送货单照片**（可多张）。若没说录到哪，系统会**问一句**：要录到哪张表 → 你发 Excel 或说表名即可。
...
- 隐式启用 `--assistant-summary-json` 和 `--review-in-excel`
...
- 若用户**自定义导入表**表头与上表**不一致**，先问清对方列名，再**口头做一一映射**（例：对方「成衣款号」↔ 本表 C「款号」），纠偏后**记入模板**（Core Rule 6）。
