...
from extract import (
    ...
    load_existing_results, merge_results,
...
from templates import (
    ...
    load_preferences, field_labels, build_learning_entries,
)
...
def process_batch(images_dir: str, output: str,
                  ...
                  ocr_retries: int = None,
                  ocr_min_confidence: float = None) -> dict:
    ...
    if ocr_retries is None:
        from extract import OCR_DEFAULT_RETRIES
        ocr_retries = OCR_DEFAULT_RETRIES
    if ocr_min_confidence is None:
        from extract import OCR_DEFAULT_MIN_CONFIDENCE
        ocr_min_confidence = OCR_DEFAULT_MIN_CONFIDENCE
    ...
    if not images:
        ...
        if Path(results_json_path).is_file():
            with open(results_json_path, "r", encoding="utf-8") as f:
                batch = json.load(f)
            return _categorize(batch, templates_dir, output,
                               export_pending=review_in_excel, profile=profile)
        return {
            "confirmed": [],
            "pending": [],
            "batch": None,
    ...
    with ThreadPoolExecutor(max_workers=parallel) as executor:
        futures = {executor.submit(extract_single, base_url, token,
                                   ...
                                   extract_progress if assistant_progress else None,
                                   profile, ocr_retries, ocr_min_confidence): img
                   for img in images}
...
def _categorize(batch: dict, templates_dir: Path, output: str,
                export_pending: bool = False, profile: dict | None = None) -> dict:
    ...
    if export_results:
        ...
        export_xlsx_append(export_batch, output, profile=profile)
...
def build_assistant_summary(result: dict, profile: dict | None = None) -> dict:
    """Build a channel-friendly summary for chat/Agent assistants.

    This is intentionally user-facing: channel handlers can send user_message
    directly without exposing paths, commands, or JSON internals. Suggested
    correction replies are derived from the active profile's vocab so they
    match the user's industry.
    """
    if profile is None:
        profile = load_profile()
    review_examples = (profile.get("vocab") or {}).get("review_examples") or ["确认", "全部确认"]
    ...
    if result.get("needs_clarification"):
        ...
        status = "needs_review"
        example_hint = "，或像「" + "」「".join(review_examples[1:3]) + "」这样纠正我" if len(review_examples) > 1 else ""
        if result.get("exported_pending"):
            message = (
                f"已先写入表格，共 {total} 张票据，其中 {len(pending_success)} 张需要你看一下。"
                f"你可以直接回复「确认」「全部确认」{example_hint}。"
            )
        else:
            message = (
...
def build_intro_summary(has_saved_table: bool = False, template_count: int = 0,
                        profile: dict | None = None) -> dict:
    if profile is None:
        profile = load_profile()
    doc_types = profile.get("document_types") or {}
    names = "、".join(dt.get("name", "") for dt in doc_types.values()) or "送货单/票据"
    review_examples = (profile.get("vocab") or {}).get("review_examples") or [
        "确认", "全部确认", "第二行数量是300"]
    example_hint = "、".join(f"「{e}」" for e in review_examples[:3])
    ...
    return {
        "status": "intro",
        "user_message": (
            f"我是票据录入助手，可以帮你把{names}等照片录进表格。"
            "你只要发清楚的票据照片，多张一起也行。"
            f"{table_text}"
            f"不对的地方直接说{example_hint}。"
...
def build_self_check_summary(check: dict) -> dict:
    if check.get("vision_supported"):
        ...
        message = (
            "安装检查未通过：当前模型看不到图片。"
            "建议先切换到支持图片的模型后再交给录入人员使用；否则只能尝试本地 OCR 兜底，速度会慢且不稳定。"
        )
        suggested = ["切换支持图片的模型", "重新检查"]
        if reason == "gateway_error":
            message = "安装检查未通过：网关暂时不可用。请先启动网关，再重新检查。"
            suggested = ["启动网关", "重新检查"]
...
def _run_interactive_loop(batch: dict, output: str, results_json: str,
                          templates_dir: Path, profile: dict | None = None) -> dict:
    """Interactive terminal mode: walk through pending items one by one."""
    if profile is None:
        profile = load_profile()
    labels = field_labels(profile)
    # Missing-field check uses the profile's item fields (skip purely-numeric
    # core that's usually optional to flag).
    check_keys = [k for k, _ in __import__("_profile", fromlist=["item_fields"]).item_fields(profile)
                  if k not in ("unit", "remark", "total_amount")]
    pending = [
    ...
    for i, result in enumerate(pending):
        ...
        if ans == "" or ans == "y" or ans == "yes":
            ...
            with open(results_json, "w", encoding="utf-8") as f:
                json.dump(batch, f, ensure_ascii=False, indent=2)
            save_from_results(results_json, None, templates_dir, profile=profile,
            ...
            field = field.strip()
            value = value.strip()
            # Map labels (incl. synonyms) to internal keys via profile + synonyms
            from templates import _synonym_map
            internal_field = _synonym_map(profile).get(field, field)
            ...
            print("  编辑模式：输入字段映射或自然语言纠正")
            print("  例: 品名=货号   表示『品名列实际是货号』")
            print("  例: 无规格      表示『这家没有规格』")
            mapping = input("  > ").strip()
            if mapping:
                if "=" in mapping:
                    k, v = mapping.split("=", 1)
                    corrections = [f"{k.strip()}就是{v.strip()}"]
                else:
                    corrections = [mapping]
                changed = apply_finalize_actions(batch, results_json, templates_dir,
                                                 corrections, supplier, profile)
...
def format_batch_review(pending_results: list, templates_dir: Path,
    ...
    if new_suppliers:
        ...
        for result in new_suppliers:
            lines.append(format_extraction_review(result, None, profile))
            lines.append("---")
            lines.append("")
    ...
    if problem_suppliers:
        ...
        for result, tmpl in problem_suppliers:
            lines.append(format_extraction_review(result, tmpl, profile))
            lines.append("---")
            lines.append("")
...
def print_summary(result: dict, templates_dir: Path = TEMPLATES_DIR,
    ...
    if pending:
        ...
        review = format_batch_review(pending, templates_dir, profile)
        if review:
            print(f"\n{review}")
            print("请逐一确认。可以回复：")
...
def _field_label_for_issue(field: str, profile: dict | None = None) -> str:
    if profile is None:
        profile = load_profile()
    return field_labels(profile).get(field, field)


def _remove_field_issue(result: dict, row_number: int, field: str,
                        profile: dict | None = None):
    label = _field_label_for_issue(field, profile)
...
def _refresh_row_update_review_status(batch: dict, correction: dict, target_supplier: str,
    ...
    for result in batch.get("results", []):
        ...
        if not _result_matches_supplier(result, target_supplier):
            continue
        _remove_field_issue(result, row_number, field, profile)
...
def apply_finalize_actions(batch: dict, results_json: str, templates_dir: Path,
                           correction_texts: list[str], target_supplier: str | None,
                           profile: dict | None = None) -> int:
    """Apply chat/Agent-style confirmation and correction texts."""
    if profile is None:
        profile = load_profile()
    corrections = []
    for text in correction_texts:
        parsed = parse_natural_correction(text, target_supplier or "", profile)
    ...
    for supplier in rule_targets:
        _events, rules = build_learning_entries(learning_by_supplier.get(supplier, []), profile)
        ...
        for result in batch.get("results", []):
            ...
            if _result_matches_supplier(result, supplier):
                ...
                post_process_extraction(result, templates_dir, profile)
    ...
    for correction in corrections:
        ...
        supplier = _resolve_action_supplier(batch, target_supplier, correction)
        if supplier:
            _refresh_row_update_review_status(batch, correction, supplier, profile)
...
def _resolve_gateway_config(args) -> tuple:
    """Resolve gateway host, port, and token.

    Priority: explicit --token > OPENAI_API_KEY (standard) > OPENAI_GATEWAY_TOKEN
    > legacy OPENCLAW_GATEWAY_TOKEN > openclaw config file (~/.openclaw/*.json).
    Host/port come from OPENAI_BASE_URL when set, else from explicit args/env,
    else defaults. This makes the skill work out-of-the-box with any
    OpenAI-compatible gateway while still auto-detecting an openclaw install.
    """
    host = args.gateway_host
    port = args.gateway_port
    token = args.token

    # Derive host/port from OPENAI_BASE_URL if present (e.g. https://host:port/v1)
    base_url_env = os.environ.get(_profile.ENV_OPENAI_BASE_URL, "")
    if base_url_env and (not host or host == DEFAULT_GATEWAY_HOST):
        import re as _re
        m = _re.match(r"^https?://([^:/]+)(?::(\d+))?", base_url_env)
        if m:
            host = m.group(1)
            if m.group(2) and port == DEFAULT_GATEWAY_PORT:
                port = int(m.group(2))
    ...
    # Standard OpenAI API key (top priority among envs)
    token = os.environ.get(_profile.ENV_OPENAI_API_KEY, "")
    if token:
        return host, port, token

    # Legacy/neutral gateway token envs
    token = (os.environ.get(_profile.ENV_GATEWAY_TOKEN, "")
             or os.environ.get(_profile.LEGACY_ENV_TOKEN, ""))
    if token:
        return host, port, token

    # Fall back to openclaw config files if present (convenience for openclaw users)
    config_paths = [
    ...
    if not token:
        print("Warning: No gateway token found. Set OPENAI_API_KEY (standard) "
              "or OPENAI_GATEWAY_TOKEN, or configure an openclaw gateway.", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--self-check", action="store_true",
                        help="Check whether the current model can see images")
    ...
    parser.add_argument("--corrections", nargs="*", default=[],
                        help="Natural language corrections (e.g., '品名就是货号' or '第二行数量是300')")
    parser.add_argument("--supplier", type=str,
                        help="Apply corrections to specific supplier")
    parser.add_argument("--gateway-host",
                        default=os.environ.get(_profile.ENV_GATEWAY_HOST,
                                    os.environ.get(_profile.LEGACY_ENV_HOST, DEFAULT_GATEWAY_HOST)))
    parser.add_argument("--gateway-port", type=int,
                        default=int(os.environ.get(_profile.ENV_GATEWAY_PORT,
                                    os.environ.get(_profile.LEGACY_ENV_PORT, str(DEFAULT_GATEWAY_PORT)))))
    parser.add_argument("--token",
                        default=os.environ.get(_profile.ENV_GATEWAY_TOKEN,
                                    os.environ.get(_profile.LEGACY_ENV_TOKEN, "")))
    parser.add_argument("--model", default=os.environ.get(_profile.ENV_OPENAI_MODEL, ""))
    parser.add_argument("--parallel", "-j", type=int, default=3)
    parser.add_argument("--retries", "-r", type=int, default=MAX_RETRIES)
    parser.add_argument("--ocr-retries", type=int, default=__import__("extract").OCR_DEFAULT_RETRIES,
                        help="Extra preprocessed OCR variants to try when confidence is low")
    parser.add_argument("--ocr-min-confidence", type=float,
                        default=__import__("extract").OCR_DEFAULT_MIN_CONFIDENCE,
                        help="Mean OCR confidence below which multi-pass retry triggers")
    ...
    parser.add_argument("--assistant-summary-json", action="store_true",
                        help="Print one channel-friendly summary JSON line for chat/Agent use")
    ...
    if args.intro:
        ...
        summary = build_intro_summary(bool(saved_table), template_count, profile)
        if args.assistant_summary_json:
            print_assistant_summary(summary, clean=args.agent_mode)
        else:
            print(summary["user_message"])
    ...
    if args.apply or args.finalize:
        ...
        if args.apply and not Path(results_json).is_file():
            result = process_batch(
                ...
                args.assistant_summary_json, profile,
                args.ocr_retries, args.ocr_min_confidence,
            )
        ...
        if args.apply:
            # Format: "供应商A:确认 | 供应商B:无规格" or just "全部确认"
        ...
        if all_corrections:
            ...
            if batch.get("needs_clarification"):
                ...
                if args.agent_mode:
                    print(json.dumps(build_assistant_summary(summary_result, profile), ensure_ascii=False, separators=(',', ':')))
                elif args.assistant_summary_json:
                    print_assistant_summary(build_assistant_summary(summary_result, profile))
                else:
                    print(batch["needs_clarification"].get("message", "请说明要修改哪一张票据。"))
                sys.exit(0)

        # Export
        export_xlsx_append(batch, output, profile=profile)
        ...
        if args.interactive and result.get("batch") and result.get("pending"):
            ...
            if not Path(results_json).is_file():
                ...
                with open(results_json, "w", encoding="utf-8") as f:
                    json.dump(result["batch"], f, ensure_ascii=False, indent=2)
            _run_interactive_loop(result["batch"], output, results_json, templates_dir, profile)
            # Export after interactive confirmations
            export_xlsx_append(result["batch"], output, profile=profile)
            set_default_table(output)
            summary = build_assistant_summary(build_result_view(result["batch"], output), profile)
            print()
            print(summary["user_message"])
        else:
            if not args.agent_mode:
                print_summary(result, templates_dir, profile)
            if args.assistant_summary_json:
                summary = build_assistant_summary(result, profile)
                if args.agent_mode:
