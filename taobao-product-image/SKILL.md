...
| 参考/借鉴这张图的风格/灯光/调色来生成 | **多参考图**（`--style-ref`，见下） |
| 手写/定制 scene prompt、文案渲染、复杂场景 | 先读 `references/prompt-craft.md` |
...
当一个商品需要"借"另一张图的**风格/灯光/调色**时，用 `--style-ref <图>`（可重复，最多 3 张额外参考）+ `--style-ref-desc "<调性描述>"`。主图（`--image`）定义商品本身，风格参考图定义视觉调性。

```bash
python3 <skill>/scripts/generate.py \
  --type scene \
  --image product.jpg \
  --style-ref lighting_reference.jpg \
  --style-ref-desc "warm creamy tone, soft frontal softbox, low-saturation palette" \
  --scene "..."
```

**规则**：
- 风格参考图只借调性，不复制其中的物体/人物。
- 主图的商品身份永远以 `--image` 为准。
- 后端支持取决于模型：OpenAI 原生 gpt-image 支持 4 图（主图+3 风格）；部分代理（如 deepkey.top）只取第一张，多余参考会被忽略（prompt 里的描述仍生效）。

---

## Prompt 工艺

手写 `--scene` / `--selling-point` / 定制 prompt 前，先读 `references/prompt-craft.md`。10 条核心规则，按 ROI 排序，最重要的几条：

1. **编辑不变量**（最重要）：图生图必须同时写"保留什么 + 改变什么"。所有内置模板已自动追加 INVARIANTS 段；用户手写 `--scene` 时提醒带上"保留"声明。
2. **结构化 prompt**：复杂场景用 JSON schema（环境/材质/灯光/道具/输出）比散文可控。
3. **画布先行**：先写比例和布局，再写主体（如 `Square 1:1 ...` → 商品）。
4. **场景密度 > 形容词**：`大理石台面、晨光斜射、白茶花` 远胜 `高端精致`。
5. **文字加引号**：卖点文案用 `"超大容量" / "长续航"` 形式，模型按字面渲染，不会臆造。
...
# 11. 多参考图：借图二的灯光/调色来生成场景图
python3 ~/.agents/skills/taobao-product-image/scripts/generate.py \
  --type scene \
  --image ~/Downloads/product.jpg \
  --style-ref ~/Downloads/lighting_reference.jpg \
  --style-ref-desc "warm creamy tone, soft frontal softbox, low-saturation milky shadows" \
  --scene "soft blush-pink bathroom vanity scene" \
  --product-hint "电动牙刷套装"

# 12. 套图 + 风格参考（每张图都借用同一风格调性）
python3 ~/.agents/skills/taobao-product-image/scripts/collection.py \
  --phase plan \
  --image ~/Downloads/product.jpg \
  --no-apparel \
  --style-ref ~/Downloads/style_ref.jpg \
  --style-ref-desc "quiet-luxury, warm marble, soft morning side light" \
  --scene "minimalist modern living room" --selling-points "蓝牙5.0, 长续航"
```
