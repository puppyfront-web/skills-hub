...
def call_openai_edits(
    ...
    image_input: local path or HTTP(S) URL — the primary product image.
    extra_images: optional list of additional paths/URLs used as style or
        composition references. Attached as repeated `image` form fields.
        OpenAI gpt-image accepts up to 4 reference images total; extra inputs
        beyond that are silently dropped. Some proxies only honor the first
        image — see references/prompt-craft.md §6 for degradation notes.
    ...
    # Attach extra reference images (style/composition refs).
    # gpt-image supports up to 4 images total (primary + 3 extras).
    if extra_images:
        cap = 3  # primary already counts as 1 of the 4-image ceiling
        for idx, ref in enumerate(extra_images[:cap]):
            try:
                r_mime, r_fname, r_data = encode_image(ref)
                # Unique field name per extra image; some backends key on
                # field name, others on order. Using image[]-style suffix.
                files[f"image{idx + 2}"] = (r_fname, r_mime, r_data)
            except Exception:
                # Skip a bad extra ref rather than failing the whole call.
                continue
