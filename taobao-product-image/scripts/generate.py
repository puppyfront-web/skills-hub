...
def generate_one(
    ...
    *,
    ...
    # Banner supports both modes: with reference image → edits; without → generations.
    has_reference_image = bool(image) and image_type != "aplus"
    ...
    # Dispatch: generations (text-to-image) when no reference image OR type is aplus.
    # edits (image-to-image) otherwise. Banner supports both based on input.
    use_generations = (image_type == "aplus") or (image_type == "banner" and not image)
    if use_generations:
        ...
        img_bytes = _gateway.call_openai_edits(
            cfg, image, prompt, cfg["image_model"], size=size, quality=quality
        )
...
def main() -> int:
    ...
    # Empty-input guard.
    # aplus always text-only; banner can be either; all others need image.
    needs_image = args.type not in ("aplus",) and not (
        args.type == "banner" and not args.image
    )
    if needs_image and not args.image:
        parser.error(f"--image 必填（type={args.type} 需要参考图；如要做无参考图 banner，可省略）")
